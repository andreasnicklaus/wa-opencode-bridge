...
| `ALLOWED_SENDERS`          | _(empty = allow all)_ | Comma-separated phone numbers (no `+`), e.g. `4915112345678,491701234567`. `@lid` senders are resolved to their phone number before matching; a sender who isn't allowed (or whose phone can't be resolved) receives a "service unavailable" reply instead of being processed. |
